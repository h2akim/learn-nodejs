...
module.exports = async (args) => {
	...
	exec(`knex migrate:make ${migrationName} --knexfile=${knexFile}`, (error, stdout, stderr) => {
		...
		const actualMigrationName = replace(fullMigrationName, `Created Migration: ${migrationDirectory}/`, '');
		console.log(`✅ Succesfully created new migration - ${actualMigrationName}`);
	});
}